...
export default class DockMediaPlayerPreferences extends ExtensionPreferences {
  fillPreferencesWindow(window) {
    ...
    const widthGroup = new Adw.PreferencesGroup({
      title: "Widget Width",
      description: "Control how wide the widget expands inside the dock",
    });
    appearancePage.add(widthGroup);

    widthGroup.add(
      this.makeSpinButtonRow(
        "Width",
    ...
    const heightGroup = new Adw.PreferencesGroup({
      title: "Widget Height",
      description: "Control how tall the widget expands inside the dock",
    });
    appearancePage.add(heightGroup);

    heightGroup.add(
      this.makeSpinButtonRow(
        "Height",
        "Height of the expanded media widget in pixels",
        settings,
        "widget-height",
        { lower: 48, upper: 180, step: 2 },
      ),
    );
    ...
    positionRow.set_selected(
      settings.get_string("widget-position") === "start" ? 0 : 1,
    );
